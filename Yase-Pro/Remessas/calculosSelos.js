...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const calcularIntervalo = () => {
        ...
        if (ini > TETO_MAXIMO || fim > TETO_MAXIMO) {
            alert(`Atenção: A numeração particular para cada prefixo deve ir até ${TETO_MAXIMO.toLocaleString()}.`);
...
async function atualizarResumoLotes() {
    try {
        ...
        if (inEstoque) {
            inEstoque.value = estoqueNota;
            inEstoque.style.backgroundColor = estoqueNota <= 10 ? '#ef4444' : '#10b981';
            inEstoque.style.color = '#fff';
        }
    } catch (err) {
