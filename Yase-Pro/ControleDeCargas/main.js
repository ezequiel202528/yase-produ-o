...
const Storage = {
  ...
  getVendedores: () =>
    JSON.parse(localStorage.getItem("vendedores")) || ["Vendedor Padrão"],
};
...
const CargaService = {
  ...
  gerarCardCarga: function () {
    ...
    const novaCarga = {
      ...
      horaGerado: agora.toLocaleTimeString("pt-BR", {
        hour: "2-digit",
        minute: "2-digit",
      }),
      dataGerado: agora.toLocaleDateString("pt-BR", {
        day: "2-digit",
        month: "2-digit",
      }),
    };
  ...
  toggleStatus: function (id) {
    ...
    if (c.status === "concluido") {
      ...
      c.status = "concluido";
      c.horaFinalizacao = agora.toLocaleTimeString("pt-BR", {
        hour: "2-digit",
        minute: "2-digit",
      });
      c.dataFinalizacao = agora.toLocaleDateString("pt-BR", {
        day: "2-digit",
        month: "2-digit",
      });
    }
  ...
  verificarAtrasosAutomaticos: function () {
    ...
    this.cargas.forEach((c) => {
      if (
        c.status === "pendente" &&
        agora - c.timestamp >= CONFIG.tempoExpiracaoAtraso
      ) {
        c.status = "atraso";
        mudou = true;
      }
...
const AppUI = {
  ...
  toggleConfigVendedores: () =>
    document.getElementById("secaoVendedores").classList.toggle("hidden"),
  ...
  atualizarInterfaceVendedores: function () {
    ...
    lista.innerHTML = CargaService.vendedores
      .map(
        (v) => `
  ...
  atualizarListaTemporaria: function () {
    ...
    if (!CargaService.itensTemporarios.length) {
      container.classList.add("hidden");
      return;
    }
    container.classList.remove("hidden");
    lista.innerHTML = CargaService.itensTemporarios
      .map(
        (item, idx) => `
  ...
  showToast: function (mensagem) {
    ...
    toast.className =
      "bg-slate-900 text-white px-6 py-4 rounded-2xl shadow-2xl text-sm font-bold animate-fade-in border border-slate-700 flex items-center gap-3";
  ...
  renderizarCards: function () {
    ...
    const filtradas = CargaService.cargas.filter(
      (c) =>
  ...
  renderizarEmptyStates: function (counts, cols) {
    const emptyConfigs = {
      atraso: {
        icon: "smile",
        text: "Tudo sob controle. Nenhuma pendência crítica por aqui.",
      },
      pendente: {
        icon: "sparkles",
        text: "Aguardando novos desafios. Que tal registrar uma carga?",
      },
      concluido: {
        icon: "rocket",
        text: "O dia promete! Vamos começar as entregas?",
      },
    };
...
window.CargaService = CargaService;
window.AppUI = AppUI;
