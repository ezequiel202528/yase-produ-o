...
async function sincronizarPainelSelos() {
    ...
    try {
        ...
        if (typeof aplicarBloqueioSistema === "function") aplicarBloqueioSistema(restante);
...
function aplicarBloqueioSistema(quantidade) {
    ...
    if (isBloqueado) {
        ...
        if (btnRegistrar) {
            ...
            btnRegistrar.innerHTML = '<i class="fa-solid fa-lock mr-2"></i> SEM SELOS DISPONÍVEIS';
        ...
        if (badgeSelo) {
            ...
            badgeSelo.parentElement.classList.replace('bg-amber-500/10', 'bg-red-500/10');
...
async function verificarDisponibilidadeSeloRealtime() {
  try {
    ...
    if (erroLote || !lote) return { podeGravar: false, motivo: "Lote não encontrado." };
