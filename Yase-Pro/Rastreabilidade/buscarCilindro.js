...
async function executarBuscaModal() {
    const nrCilindroBusca = document.getElementById('inputBuscaModal').value.trim();
        ...
        if (resultados && resultados.length > 0) {
            ...
            if (String(item.os_number) === String(osAtual)) {
                ...
                if (linhaParaDestacar) {
                    ...
                    // 3. Rola e destaca
                    linhaParaDestacar.scrollIntoView({ behavior: 'smooth', block: 'center' });
                    ...
                    alert(`✅ Localizado na OS ${osAtual}, mas não está visível na tabela.`);
                }
            } else {
    ...
    if (resultados && resultados.length > 0) {
        ...
        if (String(item.os_number) === String(osAtual)) {
            ...
            if (linhaEncontrada) {
                ...
                alert(`✅ Este cilindro está nesta OS, mas a linha não foi renderizada na tabela.`);
            }
        } else {
