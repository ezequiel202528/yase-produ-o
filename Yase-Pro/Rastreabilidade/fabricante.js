...
function abrirModalFabricante() {
    ...
    document.getElementById('modalFabricante').classList.add('flex');
    carregarFabricantes();
}

function fecharModalFabricante() {
    document.getElementById('modalFabricante').classList.add('hidden');
    document.getElementById('modalFabricante').classList.remove('flex');
}
...
function renderizarListaFabricantes(lista) {
    ...
    container.innerHTML = lista.map(fab => `
...
// Cadastrar novo
// 1. Renderiza a lista no modal (ID | Nome)
async function carregarFabricantes() {
    const { data, error } = await supabase
        .from('fabricantes')
        .select('id, nome')
    ...
    container.innerHTML = data.map(fab => `
        <div class="flex items-center gap-3 p-3 bg-slate-800/30 border border-slate-700/30 rounded-xl hover:border-amber-500/50 transition-all">
            <span class="bg-amber-500/10 text-amber-500 font-black text-[10px] px-2 py-1 rounded-md">ID: ${fab.id}</span>
...
document.getElementById('X_input_id').addEventListener('blur', async function() {
...
function selecionarFabricante(id, nome) {
    ...
    inputFabId.classList.add('border-emerald-500', 'ring-2', 'ring-emerald-500/20');
    setTimeout(() => inputFabId.classList.remove('border-emerald-500', 'ring-2', 'ring-emerald-500/20'), 1000);
...
function exibirAlertaErro(mensagem) {
    ...
    setTimeout(() => {
        if (modal) {
            modal.classList.add('animate-out', 'fade-out', 'zoom-out', 'slide-out-to-top-8');
